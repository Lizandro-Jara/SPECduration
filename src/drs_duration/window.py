...
DEFAULT_min_points_in_window = 15
DEFAULT_derivative_reference_percentile = 95
...
def detect_window_by_derivative(
    ...
    """
    Detects the predominant DRS window around the real peak using a robust
    derivative-based criterion:

        |dR/dT| <= alpha * P95(|dR/dT|)

    where the derivative reference is computed locally around the DRS peak.
    """
    ...
    ref_abs_dR = float(
        np.percentile(
            np.abs(dR[inds_search]),
            DEFAULT_derivative_reference_percentile,
        )
    )

    threshold = float(alpha) * ref_abs_dR
    ...
    while iL > 0:
        ...
        if gap_count > DEFAULT_allowed_gap:
            break

        iL -= 1

    iL = min(iL + DEFAULT_allowed_gap, idx_peak)
    ...
    iR = max(iR - DEFAULT_allowed_gap, idx_peak)
